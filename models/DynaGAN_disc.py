...
class DynaGAN(torch.nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            ZP_target_latent, new_img, ZP_img_tensor_256, ZP_img_clip_embed = ZP_target_latent.cuda(), new_img.cuda(), ZP_img_tensor_256.cuda(), ZP_img_clip_embed.cuda()
        ...
        if inference:
            with torch.no_grad():
                rec_img = self.generator_trainable([self.ZP_target_latent.cuda()], input_is_latent=True, truncation=1, domain_labels = [domain_labels[0][0:1].repeat(len(self.ZP_target_latent), 1)],
                                                   randomize_noise=randomize_noise,domain_is_latents=True)[0]
            ...
            if self.args.lambda_contrast > 0.:
                contrastive_loss = torch.sum(torch.stack(
                    [self.clip_model_weights[model_name] * self.clip_loss_models[model_name].contrastive_adaptation_loss(trainable_img, self.ZP_img_tensor_256.cuda(), domain_labels_c) for
                    model_name in self.clip_model_weights.keys()]))
            else:
                contrastive_loss = 0.
